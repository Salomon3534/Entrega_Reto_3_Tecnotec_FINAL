...
public class ViewEuskalEncounter {
		...
		while (true) {
			try {
				...
				if (texto.trim().isEmpty())
					throw new Exception("La entrada no puede estar vac�a.");
				if (texto.length() > maxLength)
					throw new Exception("Longitud m�xima: " + maxLength);
				if (!texto.matches("[a-zA-Z������������ 0-9\\- .:,;]+"))
	...
	private String readDni() {
		while (true) {
			try {
				System.out.print("DNI (8 n�meros + 1 letra): ");
	...
	public void mainMenu() {
		...
		do {
			System.out.println("\n========================================");
			System.out.println(" SISTEMA DE GESTI�N DE EUSKAL ENCOUNTER");
			System.out.println("========================================");
	...
	/**
	 * Muestra el men� de acciones. Permite definir si se incluye la opci�n de ID
	 * Global.
	 */
	private int showActionMenu(String title, boolean showExtended) {
		System.out.println("\n>> GESTI�N DE " + title.toUpperCase());
		...
		if (showExtended) {
			System.out.println("6. Ver pr�ximo ID disponible");
			System.out.print("Acci�n (0-6): ");
			return readInt(0, 6);
		} else {
			System.out.print("Acci�n (0-5): ");
			return readInt(0, 5);
		}
	}
	...
	private void manageEvents() {
		...
		do {
			...
			try {
				...
				case 6 -> System.out.println("Pr�ximo ID Evento: " + totalManager.getGlobalCounterEvent());
				}
			} catch (Exception e) {
	...
	private void manageEncounters() {
		...
		do {
			...
			try {
				switch (action) {
				case 1 -> {
					String loc = readString("Ubicaci�n: ", 150);
				...
				case 3 -> {
					...
					String loc = readString("Nueva Ubicaci�n: ", 150);
				...
				case 6 -> System.out.println("Pr�ximo ID Encuentro: " + totalManager.getGlobalCounterEncounter());
				}
			} catch (Exception e) {
				System.err.println("Error: " + e.getMessage());
			}
	...
	private void manageGuests() {
		int action;
		do {
			action = showActionMenu("Invitados", false); // Opci�n 6 desactivada
			if (action == 0)
				break;
			try {
				switch (action) {
				case 1 -> System.out.println(totalManager.createGuest(readString("Usuario: ", 50),
						readString("Nombre: ", 100), readString("Apellidos: ", 150), readString("Tel�fono: ", 15),
	...
	private void manageUsers() {
		int action;
		do {
			action = showActionMenu("Usuarios", false); // Opci�n 6 desactivada
			if (action == 0)
				break;
			try {
				switch (action) {
	...
	private Event requestEventData(int id, int typeChoice) {
		String title = readString("T�tulo: ", 100);
		String loc = readString("Ubicaci�n: ", 150);
		String desc = readString("Descripci�n: ", 255);
